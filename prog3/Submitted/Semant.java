...
// import Translate.Exp; /* choosing to explicitly state Translate.Exp in order to use Absyn.* with no Exp confusion */
import Types.*;
import Absyn.*;
...
public class Semant {
  ...
  public void transProg(Exp exp) { // Absyn.Exp
    transExp(exp);
  }
...
/* helper methods */
  ...
  private Translate.Exp checkComp(ExpTy et, int pos) {
    Type a = et.ty.actual();
    if ((!(a instanceof INT)) && 
        (!(a instanceof STRING)) && 
        (!(a instanceof NIL)) && 
  ...
  private Translate.Exp checkOrder(ExpTy et, int pos) {
    Type a = et.ty.actual();
    if ((!(a instanceof INT)) && 
        (!(a instanceof STRING))) {
...
/* expressions */  
  ExpTy transExp(Exp e) { // Absyn.Exp
    ExpTy result;
  ...
  ExpTy transExp(OpExp e) {
    ...
    case 5: 
      ...
      if ((!left.ty.coerceTo(right.ty)) && (!right.ty.coerceTo(left.ty))) {
        error(e.pos, "incompatible operands for equality operator");
      }
      return new ExpTy(null, INT);
    case 6: 
    case 7: 
    case 8: 
    case 9: 
      checkOrder(left, e.left.pos);
      checkOrder(right, e.right.pos);
      if ((!left.ty.coerceTo(right.ty)) && (!right.ty.coerceTo(left.ty))) {
        error(e.pos, "incompatible operands for inequality operator");
      }
      return new ExpTy(null, INT);
    }
  ...
  Translate.Exp transDec(FunctionDec d) {
    ...
    for (FunctionDec f = d; f != null; f = f.next) {
      if (hash.put(f.name, f.name) != null) {
        error(f.pos, "function already declared");
      }
  ...
  Translate.Exp transDec(TypeDec d) {
    ...
    for (TypeDec type = d; type != null; type = type.next) {
      if (hash.put(type.name, type.name) != null) {
        error(type.pos, "type already declared");
      }
...
/* more helpers */   
  private RECORD transTypeFields(Hashtable hash, FieldList f) {
    ...
    if (hash.put(f.name, f.name) != null) {
      error(f.pos, "function parameter/record field already declared: " + f.name);
    }
...
/* additional classes (in Semant.java to minimize # of files) */
